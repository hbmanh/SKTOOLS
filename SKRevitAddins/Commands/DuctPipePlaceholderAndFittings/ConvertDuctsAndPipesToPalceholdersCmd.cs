            ...
            using (Transaction trans = new Transaction(doc))
            {
                ...
                foreach (var duct in mepCurves.OfType<Duct>())
                {
                    ...
                    if (startPoint != null && endPoint != null)
                    {
                        ...
                        newDuctPlaceholder.get_Parameter(BuiltInParameter.RBS_OFFSET_PARAM)?.Set(Utils.UnitUtils.MmToFeet(offset));
                        doc.Delete(duct.Id);
                    }
                }
                ...
                foreach (var pipe in mepCurves.OfType<Pipe>())
                {
                    ...
                    if (startPoint != null && endPoint != null)
                    {
                        ...
                        newPipePlaceholder.get_Parameter(BuiltInParameter.RBS_OFFSET_PARAM)?.Set(Utils.UnitUtils.MmToFeet(offset));
                        doc.Delete(pipe.Id);
                    }
                }
        ...
        private double GetCurrentOffsetForSystem(Document doc, ElementId systemId, List<MEPCurve> mepCurves)
        {
            ...
            foreach (var mepCurve in mepCurves)
            {
                ...
                if (currentSystemId == systemId)
                {
                    ...
                    if (offset.HasValue)
                    {
                        return Utils.UnitUtils.FeetToMm(offset.Value); // Convert from feet to mm
                    }
                }
            }
